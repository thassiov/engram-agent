...
```
┌──────────────────────┐          ┌──────────────────────────┐
│   Claude Code         │          │   engram serve            │
│                       │   hook   │   (3rd party)             │
│   UserPromptSubmit ───┼──POST──▶│                           │
│   Stop             ───┼──POST──▶│   • MCP server (plugin)   │
│                       │          │   • SQLite + FTS5 search  │
└──────────────────────┘          │   • CLI + TUI             │
                                   └──────────┬───────────────┘
                                              │ HTTP API
...
│   │              │◀───│              │                       │   │
│   │ push (30s)   │    │ LISTEN/NOTIFY│                       │   │
│   │ pull (notify) │    │ scope filter │                       │   │
│   └──────────────┘    └──────────────┘                       │   │
│                                                              │   │
│   ┌──────────────┐                           ┌─────────────┐│   │
│   │ Internal DB  │                           │ ollama       ││   │
...
```
Machine A (portus)                    Machine B (pharos)
┌──────────────┐                     ┌──────────────┐
│ engram.db    │                     │ engram.db    │
│ (SQLite)     │                     │ (SQLite)     │
│              │                     │              │
...
│                                                  │
│  engram_sync_mutations   (append-only CDC log)   │
│  engram_sync_cursors     (per-machine watermarks)│
│  engram_machines         (registry + scope)      │
│                                                  │
│  NOTIFY 'engram_sync' on INSERT (real-time)      │
└──────────────────────────────────────────────────┘
       │ LISTEN + poll (60s)                │
       ▼                                    ▼
  Machine A pulls B's mutations       Machine B pulls A's mutations
